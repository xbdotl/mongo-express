...
let connect = function (config) {
  ...
  // reference: http://mongodb.github.io/node-mongodb-native/2.2/api/Server.html
  let dbOptions = {
    
    // `auto_reconnect` may needs to be removed
    auto_reconnect: config.mongodb.autoReconnect,

    poolSize:       config.mongodb.poolSize,
    ssl:            config.mongodb.ssl,
    sslValidate:    config.mongodb.sslValidate,
    checkServerIdentity: config.mongodb.checkServerIdentity,
    sslCA:          config.mongodb.sslCA,
    sslCert:        config.mongodb.sslCert,
    sslKey:         config.mongodb.sslKey,
    sslPass:        config.mongodb.sslPass,
    servername:     config.mongodb.servername,

    // just append default configurations supported by mongodb native driver
    // TODO: configurable
    socketOptions: {
      autoReconnect: true,
      noDelay: true,
